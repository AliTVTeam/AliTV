var width = 1200, height = 800, innerRadius = Math.min(width, height) * 0.41, outerRadius = innerRadius * 1.1;
var identity = {
	percent : [ "Identity", "<46 %", "50 %", "54 %", "58 %", "62 %", "66 %",
			"70 %", "74 %", "78 %", "82 %", "86 %", "90 %", "94 %", ">98 %" ],
	color : [ "white", "#FF1600", "#FF3500", "#FF5300", "#FF7C01", "#FF9B01",
			"#FFC301", "#FFE201", "#EBDD02", "#CCD602", "#B7D103", "#99C905",
			"#7AC206", "#51B807", "#32B008" ]
};
...
var legend = svg.selectAll('.legend').data(identity.percent).enter()
...
for (var i = 0; i <= 14; i++) {
	...
	legend.append('rect').attr('x', pos_x).attr('y', pos_y).attr('width',
			legendRectSize).attr('height', legendRectSize).style('fill',
			identity.color[i]).style('stroke', identity.color[i]);

	legend.append('text').attr('x', pos_x_text + legendSpacing).attr('y',
			pos_y_text - legendSpacing).text(identity.percent[i]);
}
...
function addTicks(array) {
	var ticks = svg.append("g").attr("class", "ticks").selectAll("g").data(
...
function add_tooltip_legend(g) {
	...
	div.transition().duration(200).style("opacity", 0.9).style("left",
			(d3.event.pageX - 34) + "px").style("top",
