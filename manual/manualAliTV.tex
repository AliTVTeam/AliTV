...
If you want to reset the zoom, click right on the chromosome and select \menu{Reset genome zoom/pan}.

\subparagraph*{Shifting}
For analyzing circular sequences it may be helpful to rotate them. Therefore go to \menu{Advanced Settings}. Via the selection box you can select a chromosome you want to shift. As you can see a button group appears next to the selected chromosome. By using the right or the left arrwo you can shift the chromosome. If a link or a feature has an overlap it will be splitted into two parts. 

Type in an new value in the input box and the distance for shifting chromosomes has changed. Remove the offset by clicking \menu{Remove offset}.
